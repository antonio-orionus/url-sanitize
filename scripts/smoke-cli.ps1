...
$ActualCatalogDate = if ($Matches[3]) { $Matches[3] } else { "" }
...
if ($ActualCatalogDate -and $ActualCatalogDate -notmatch '^[0-9]{4}-[0-9]{2}-[0-9]{2}T.*Z$') {
  throw "unexpected catalog date: $ActualCatalogDate"
}

if ($ExpectedVersion -and $ExpectedVersion -notmatch '^0\.' -and -not $ActualCatalogDate) {
  throw "missing catalog date in --version output: $VersionOutput"
}
...
$CatalogLabel = $ActualCatalogHash
if ($ActualCatalogDate) {
  $CatalogLabel = "$CatalogLabel $ActualCatalogDate"
}

Write-Host "smoke passed for $Command ($ActualVersion catalog $CatalogLabel)"
